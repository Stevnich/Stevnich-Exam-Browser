	...
	public class ProcessFactory : IProcessFactory
	{
		...
		private System.Diagnostics.Process StartNormal(string path, params string[] args)
		{
         var process = new System.Diagnostics.Process();
			...
			catch (Win32Exception ex) when (ex.NativeErrorCode == 740 || ex.Message?.IndexOf("requires elevation", StringComparison.OrdinalIgnoreCase) >= 0)
			{
				// The target executable requires elevation. Retry using Shell execution with the 'runas' verb
				...
				try
				{
					var elevated = System.Diagnostics.Process.Start(startInfo);
					return elevated;
				}
				catch (Exception inner)
				{
					logger.Error($"Failed to start elevated process '{path}'!", inner);
					throw;
				}
			}
		}
