...
namespace SafeExamBrowser.UserInterface.Desktop.Controls.ActionCenter
{
	internal partial class AudioControl : UserControl, ISystemControl
	{
		...
		private void InitializeAudioControl()
		{
			...
			Button.MouseWheel += Volume_MouseWheel;
		...
		{
			// 2. Cegah eksekusi jika perubahan berasal dari internal (UpdateVolume)
			if (_isInternalUpdate) return;

			// Set volume ke backend secara real-time
			audio.SetVolume(Volume.Value / 100);

			// Update label dan icon secara real-time tanpa menyentuh Volume.Value
		...
		private void UpdateVolume(double volume, bool muted)
		{
			...
			// 3. Gunakan flag saat mensinkronisasi nilai dari sistem ke Slider
