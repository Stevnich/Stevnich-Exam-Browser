...
namespace SafeExamBrowser.UserInterface.Desktop.Windows
{
	internal partial class ActionCenter : Window, IActionCenter
	{
		...
		int ShowDurationPref, inAnimDur, outAnimDur;
		...
		void initShowDurationPref()
		{
			...
			// Combine everything into the final directory path
			string dir = Path.Combine(roaming, "SafeExamBrowser", "p_cfg");
			string filePath = Path.Combine(dir, "actCenter");
			...
			try
			{
				// 1. Ensure the entire directory tree exists
				...
				// Assign values
				ShowDurationPref = (int) double.Parse(lines[0], System.Globalization.CultureInfo.InvariantCulture);
				inAnimDur = (int) double.Parse(lines[1], System.Globalization.CultureInfo.InvariantCulture);
				outAnimDur = (int) double.Parse(lines[2], System.Globalization.CultureInfo.InvariantCulture);
			}
			catch
			{
				// Log the error if possible, otherwise fall back to hardcoded defaults in memory
		...
		private void HideAnimated()
		{
			...
			var animation = new DoubleAnimation
			{
				...
				From = 0,
				To = -Width,
				Duration = new Duration(TimeSpan.FromMilliseconds(outAnimDur))
			};
		...
		private void ShowAnimated()
		{
			...
			var animation = new DoubleAnimation
			{
				...
				From = -Width,
				To = 0,
				Duration = new Duration(TimeSpan.FromMilliseconds(inAnimDur))
			};
